// 1. Pastikan ini ada di paling atas file script.js
...
function renderReviews(isAdmin) {
    ...
    try {
        // Hentikan listener lama jika ada untuk mencegah memory leak
        ...
        reviewUnsubscribe = firebase.firestore().collection('reviews')
            ...
            .onSnapshot((snapshot) => {
                ...
                if (error.code === 'permission-denied') {
                    testimonialGrid.innerHTML = '<p style="text-align:center; color:var(--text-slate);">Menunggu izin akses...</p>';
                } else {
                    testimonialGrid.innerHTML = '<p style="text-align:center; color:red;">Gagal memuat ulasan.</p>';
                }
            });
    } catch (e) {
        console.error("Error initializing reviews:", e);
    }
}
...
function initFirestoreListeners() {
    ...
    firebase.firestore().collection('categories').onSnapshot((snapshot) => {
        ...
        if (error.code === 'permission-denied') {
            showNotification("Akses Ditolak", "Gagal memuat kategori (Izin ditolak).", "error");
        }
    });
    ...
    firebase.firestore().collection('products').onSnapshot((snapshot) => {
        ...
        if (error.code === 'permission-denied') {
            showNotification("Akses Ditolak", "Gagal memuat produk (Izin ditolak).", "error");
        }
    });
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 1. Initialize Firestore Listeners (ALWAYS FIRST)
    if (document.getElementById('product-grid') && typeof firebase !== 'undefined') {
    ...
    // Load initial data logic moved to top of DOMContentLoaded
    // ...
    ...
    // Logic has been moved to top global scope as requested.
});
